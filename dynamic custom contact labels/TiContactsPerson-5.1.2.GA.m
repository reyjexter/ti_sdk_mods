...
+(NSDictionary*)multiValueLabels
{
    if (multiValueLabels == nil) {
        ...
        [[NSDictionary alloc] initWithObjectsAndKeys:(NSString*)kABHomeLabel,@"home", // Generic labels
         ...
         CFSTR("Resume/CV/Bio"),@"resume",
         CFSTR("LinkedIn Profile"),@"linkedInProfile",
         nil];
    }
...
{
    // Modified by Rey to support custom labels
    NSLog(@"[INFO] Debugging contact with custom label...");
    ...
    for (NSString* key in [dict allKeys]) {
        ...
        for (id value in [dict objectForKey:key]) {
            ...
            if (type == kABMultiDateTimePropertyType) {
                ...
                NSLog(@"[INFO] Adding custom label");
...
{
    // Modified by Rey to support custom labels
    NSLog(@"[INFO] Set value called. key = %@, value = %@", key, value);
        ...
        if ([key isEqualToString:@"phone"]) {
            ...
            for (NSString *key in keys) {
                ...
                for (NSString *object in objects) {
                    ...
                    // Modified by Rey to support custom labels
                    NSString* readableLabel = nil;
                    NSArray* labelKeys = [[TiContactsPerson iOS9multiValueLabels] allKeysForObject:key];
                    if (labelKeys != nil && ([labelKeys count] > 0)) {
                        readableLabel = [labelKeys objectAtIndex:0];
                    }
                    else {
                        if (key == NULL) {
                            ...
                            readableLabel = (NSString*)key;
                        }
                    }
                    
                    CNLabeledValue *labeledValue = [CNLabeledValue labeledValueWithLabel:readableLabel value:phoneNumber];
                    // end modify by rey
        ...
        if ([key isEqualToString:@"email"]) {
            ...
            for (NSString *key in keys) {
                ...
                for (NSString *object in objects) {
                    ...
                    CNLabeledValue *labeledValue = [CNLabeledValue labeledValueWithLabel:readableLabel value:object];
                    // end modify by rey
